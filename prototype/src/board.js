...
(function () {
  ...
  const LAST_LAP = ["good", "late", "good", "early", "good", "veryLate", "good", "early", "good"];
  ...
  const Z3 = verdict("good", -0.03, -2, 0.61, 0.62, 3);
  const base = {
  ...
  [
    ...
    { tag: "−3 s", title: "3", state: S({ beat: 3, fill: 0.08, dist: 186 }), note: "The count starts 3 s out (a setting)." },
    { tag: "−2 s", title: "2", state: S({ beat: 2, fill: 0.4, dist: 124 }), note: "One segment per count; the fill moves continuously." },
    { tag: "−1 s", title: "1", state: S({ beat: 1, fill: 0.82, dist: 40 }), note: "Last count: the fill turns red." },
    { tag: "0 s", title: "BRAKE", state: S({ mode: "brake", beat: 0, fill: 1, dist: 0 }), note: "At the reference brake point. Flashes, then holds solid." },
    { tag: "Braking", title: "On the brake", state: S({ mode: "braking", beat: 0, fill: 1, live: 0.66, peak: 0.66, verdict: verdict("good", 0.02, 1, 0.66, 0.72, 4, { current: true }), pips: pips(["good", "late", "good", "good"]) }),
      note: "Graded the moment you brake. The gauge fills toward the target line." },
    { tag: "Released", title: "Result", state: S({ mode: "idle", beat: 0, fill: 0, zoneNo: 5, dist: 412, target: 0.48, verdict: verdict("good", 0.02, 1, 0.7, 0.72, 4), pips: pips(["good", "late", "good", "good"], 4) }),
      note: "Holds until the next zone is decided; the pip for Z4 turns green." },
  ...
  $("#scale").innerHTML = [
    ...
    ["good", `within ±${tol.toFixed(2)} s`, "On the reference brake point."],
  ...
  [
    { tag: "Early", title: "Braked before the count ended",
      ...
      note: "The bar stops where you braked, in the early colour; the gap to the cap is how early." },
  ...
  const cfg = { lead: 3, early: 0, min: 0.15, tol };
  let st = null;
